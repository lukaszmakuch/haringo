...
class FactoryObjectProductBuilderTest extends PHPUnit_Framework_TestCase
{
    public function testBuildingFromDeserializedPlan()
    {
        ...
        $planMapper->registerActualMapper(
            new \lukaszmakuch\ObjectBuilder\BuildPlanMapper\Impl\FactoryObjectProductBuildPlanMapper(
                $valueSourceArrayMapper,
                $methodCallMapper
            ),
        ...
        $plan->setFactoryObject(
            ...
            new BuildPlanValueSource((new NewInstanceBuildPlan())
                ->setClassSource(new ExactClassPath(TestStaticFactory::class)
            ))
        );
        ...
        $serializedPlan = $serializer->serialize($plan);
        $deserializedPlan = $serializer->deserialize($serializedPlan);
        $builtObject = $builder->buildObjectBasedOn($deserializedPlan);
